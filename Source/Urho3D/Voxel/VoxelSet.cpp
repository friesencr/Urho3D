...
void BuildChunkWorkHandler(const WorkItem* item, unsigned threadIndex)
{
    ...
    workLoad->set_->BuildChunkWork(workLoad->chunk_);
}
...
void VoxelSet::BuildChunk(VoxelChunk* chunk)
{
    ...
    workItem.aux = workLoad; 
    workItem.workFunction_ = BuildChunkFragment;
    BuildChunkFragment(workItem, 0);
}

void VoxelSet::DecodeToVertexBuffer(VoxelWorkload* worker)
{
    /* worker->build_buffer */
}
...
{
    SharedPtr<VoxelChunk> chunk(new VoxelChunk(context_));
    chunk->SetIndex(0,0,0);
    chunk->SetSize(
    BuildChunk(chunk);
}
